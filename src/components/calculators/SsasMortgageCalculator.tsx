...
import {
  calculateSsasMortgage,
  type SsasMortgageInputs,
} from "@/lib/calculators/ssas-mortgage";
import {
  BarChart,
  Bar,
  XAxis,
...
export function SsasMortgageCalculatorClient() {
  const [inputs, setInputs] = useState<SsasMortgageInputs>({
    propertyValue: 500000,
    ssasFundValue: 400000,
    ltvPercent: 50,
    interestRate: 5.49,
    termYears: 20,
  });

  const results = calculateSsasMortgage(inputs);
    ...
    <div className="grid lg:grid-cols-5 gap-8">
      ...
      >
        ...
        <div>
          ...
          <Input
            type="number"
            value={inputs.ssasFundValue}
            onChange={(e) =>
              setInputs({ ...inputs, ssasFundValue: Number(e.target.value) })
            }
            className="glass-input text-white"
          />
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          <div className="glass-card p-5">
            ...
            <p className="text-xs text-white/30 mt-1">From SSAS fund</p>
          </div>
          <div className="glass-card p-5">
            <p className="text-xs text-white/40 uppercase tracking-wider mb-1">
              Total Cost
          ...
          <div className="glass-card p-5">
            ...
            <p
              className={`text-xl font-bold ${
                inputs.ssasFundValue - results.depositRequired >= 0
                  ? "text-emerald-400"
                  : "text-red-400"
              }`}
            >
              {fmt(
                Math.max(0, inputs.ssasFundValue - results.depositRequired)
              )}
            </p>
          </div>
        </div>
